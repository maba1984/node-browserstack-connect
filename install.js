...
var AdmZip = require('adm-zip'),
    ...
    url = require('url'),
    util = require('util'),
    helper = require('./lib/BrowserStackLocal'),
    libPath = path.join(__dirname, 'lib', 'BrowserStackLocal'),
    downloadUrl = 'http://www.browserstack.com/browserstack-local/BrowserStackLocal-' + '%s%s.zip',
...
downloadUrl = util.format(downloadUrl, platform, platformType);
...
npmconf.load(function(err, conf) {
  ...
  .then(function () {
    console.log('Done. BrowserStackLocal binary available at', helper.path);
  })
  .fail(function (err) {
    console.error('BrowserStackLocal installation failed', err.stack);
    process.exit(1);
  })
})
...
function findSuitableTempDirectory(npmConf) {
  ...
  for (var i = 0; i < candidateTmpDirs.length; i++) {
    var candidatePath = path.join(candidateTmpDirs[i], 'BrowserStackLocal');
  ...
  console.error('Can not find a writable tmp directory, please report issue on http://www.browserstack.com/support with as much information as possible.');
  process.exit(1);
}
